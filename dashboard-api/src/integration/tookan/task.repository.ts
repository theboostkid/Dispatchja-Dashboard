...
export class TaskRepository {
  ...
  async createMany(tasks: Task[]) {
    return this.taskModel.bulkWrite(
      tasks.map((task) => ({
        updateOne: {
  ...
  async update(
    tasksFilterQuery: FilterQuery<TaskDocument>,
    task: Partial<Task>,
  ): Promise<Task> {
    return this.taskModel.findOneAndUpdate(tasksFilterQuery, task, {
      new: true,
    });
  }
}
