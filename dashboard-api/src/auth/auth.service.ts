...
export class AuthService {
  ...
  public async sentNewPassword(resetPasswordToken: string) {
    const user = await this.usersService.findUserByResetToken(
      resetPasswordToken,
    );
    if (!user || user.isDeleted || !user.isActive) {
  ...
  public getJwtCookie(payload: any): { token: string; tokenInCookie: string } {
    ...
    const hourOfDay = this.configService.get<number>(
      'JWT_ACCESS_TOKEN_EXPIRATION_HOUR',
    );
    ...
    return {
      token,
      tokenInCookie: `AccessToken=${token}; HttpOnly; Path=/; Max-Age=${expiresIn}`,
    };
  }
