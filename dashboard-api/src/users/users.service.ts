...
import { CreateUserDto, UserSearchQueryParams, UpdateUserDTO } from './dto/user.dto';
...
export class UsersService implements OnModuleInit {
	...
	async checkIfUserAlreadyExist(userId: string, user: CreateUserDto | UpdateUserDTO) {
		...
		if (tookanUserId) {
			...
			if (existingUser && userId !== existingUser.id && existingUser.tookanUserId) {
	...
	async updateLastLogInDate(id: string, date: string) {
		return this.usersRepository.update({ id }, { loginAttempts: 0, lastLoginDate: date })
	}
