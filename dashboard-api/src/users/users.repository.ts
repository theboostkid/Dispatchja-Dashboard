...
export class UsersRepository {
	...
	async findAll(usersFilterQuery: FilterQuery<UserDocument>, skip: number = 0, limit: number = 0): Promise<{ results: User[], count: number }> {
		const countQuery = this.userModel.find(usersFilterQuery);
		const query = this.userModel.find(usersFilterQuery).sort({ _id: 1 }).skip(skip * limit)
